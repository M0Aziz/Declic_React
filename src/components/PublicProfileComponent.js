// PublicProfileComponent.js

import React, {useState} from 'react';
import { useNavigate } from 'react-router-dom'; 
import Modal from 'react-modal';
import { FontAwesomeIcon } from '@fortawesome/react-fontawesome';
import { faArrowLeft, faArrowRight, faExclamationTriangle, faInfoCircle, faTimes } from '@fortawesome/free-solid-svg-icons';
import axios from 'axios';
import { ToastContainer, toast } from 'react-toastify';
import 'react-toastify/dist/ReactToastify.css';

const PublicProfileComponent = ({ user, followersCount, followingCount, isFollower,isFollowing, RemoveFollowers, AddFollowers, userId, username,isLoading }) => {
            ...
            <div className="card border rounded shadow p-3 mb-5 bg-body rounded mt-5 ">
              <div className="row">
                ...
                {user.user.profilePicture.startsWith('https://') ? (
                  // Si l'URL de l'image commence par "https://", affichez l'image normalement
                  ...
                  // Si l'URL de l'image ne commence pas par "https://", utilisez l'URL de votre serveur local
              ...
              <div className='col-md-12 d-flex justify-content-center'>
                {isLoading ? (
                  <button className="btn btn-primary" disabled>Chargement...</button>
                ) : (
                  <React.Fragment>
                    {isFollower ? (
                ...
                {isFollowing && isFollower ? (
                  <button className="btn btn-outline-custom-secondaire  me-4 " onClick={() => onMessageButtonClick(username)}>Message</button>
                ) : (
                  <button className="btn btn-outline-custom-secondaire  me-5 " onClick={() => onMessageButtonClick(username)} disabled>Message</button>
                )}
            
                <button type="button" className="btn btn-secondaire ms-lg-5 mx-2" data-bs-toggle="modal" data-bs-target="#exampleModal2">:</button>
